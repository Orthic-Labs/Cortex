...
import { existsSync, mkdirSync, copyFileSync, readFileSync, writeFileSync, readdirSync, statSync, cpSync } from "node:fs";
...
export function stageRuntime({ out = null } = {}) {
  const outDir = out ?? join(ROOT, "release", "runtime", `${process.platform}-${process.arch}`);
  ...
  const temp = join(tmpdir(), `cortex-runtime-stage-${process.pid}`);
  mkdirSync(temp, { recursive: true });
  execFileSync("npm", ["pack", "--pack-destination", temp], { cwd: ROOT, stdio: "ignore" });
  ...
  execFileSync("npm", ["install", "--omit=dev", "--no-audit", "--no-fund"], { cwd: packageDir, stdio: "ignore", timeout: 240000 });
  ...
  for (const name of readdirSync(launcherSrc)) {
    ...
    if (name !== "cortex.cmd") execFileSync("chmod", ["+x", join(binDir, name)]);
  }
  ...
  execFileSync("rm", ["-rf", temp]);

  return {
    root: outDir,
    layout: ["bin/cortex", "bin/cortex.cmd", "bin/cortex-mcp", "lib/node", "app/package", "app/node_modules", "app/grammars", "app/schemas", "LICENSE", "THIRD_PARTY_NOTICES", "README.txt"],
    version: pkg.version,
