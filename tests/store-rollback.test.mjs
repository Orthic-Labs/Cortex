...
test("update-chain rollback restores a migrated store to its prior line with data intact", () => {
  ...
  try {
    ...
    copyFileSync(v13, join(updateRoot, ".agent", "graph", "graph.db"));
    ...
    try {
      assert.equal(getFile(restoredDb, "a.ts").content_hash, "h-a-v13", "rollback restores the post-migration data line");
    } finally {
...
test("N-2 store rolls back cleanly to its own line through the backup mechanism", () => {
  ...
  try {
    ...
    closeStore(db);
    const backup = migrationBackupPath(v12, 12);
    assert.ok(existsSync(backup), "N-2 backup exists");
    rmSync(v12, { force: true });
    const repaired = repairInterruptedMigration(v12, 12);
    assert.equal(repaired.restored, true);
    const reopened = openStore(v12);
    try {
      assert.equal(getSchemaVersion(reopened), SCHEMA_VERSION);
      assert.equal(getFile(reopened, "b.ts").content_hash, "h-b-v12");
