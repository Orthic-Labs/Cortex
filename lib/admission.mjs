...
// Returns a neutral decision contract MemRight (or any host) can consume.
// Does NOT install hooks, classify shell, or fail-closed on tool use.

import { createHash } from "node:crypto";
import { basename, dirname, normalize as normalizePathPosix, resolve } from "node:path";
...
export function createAdmission(options = {}) {
  ...
  async function orient(input = {}) {
    ...
    if (input.freshnessReceipt) candidateSet.freshnessReceipt = input.freshnessReceipt;
  ...
  async function expand(input = {}) {
    ...
    for (const p of relativePaths) {
      if (normalizePathPosix(p).startsWith("/") || /^[A-Za-z]:[\\/]/.test(p)) {
        return decision({
          action: "block",
