...
// resistance: the blueprint→MemRight federation provider reads only the
// generation identity, never validates it cryptographically
// (tools/memright/federation/providers/blueprint.py). hash-wasm's own
...
// Legacy SHA-256 form, kept ONLY so a Phase-4 user decision recorded before
// the xxh3 migration can still be recognized as current. `isReconciliationDecisionCurrent`
// gates a STORED USER DECISION (case c) — unlike every other fingerprint in
// this module, changing the algorithm here would silently re-open every
// already-settled reconciliation decision on the next run. Never used to
// MINT a new fingerprint (sealPhase2Artifacts / reconcileInputFingerprint
// always write the xxh128: form); used only to RECOGNIZE an old one.
function legacyFingerprintSha256(value) {
  // MUST stay sha256. This is the one hash in blueprint that is READ, not
  // minted: it reproduces the pre-migration digest so a decision sealed before
  // the xxh3 migration is still recognised. Computing xxh128 here would make
  // every stored decision's fingerprint mismatch, silently re-opening every
  // reconciliation the operator already settled. Reading legacy data requires
  // the legacy algorithm — that is not migration debt, it is the migration.
...
// Legacy SHA-256 counterpart of the above, over the identical payload shape.
// Used ONLY by isReconciliationDecisionCurrent to recognize a decision that
// was sealed before the xxh3 migration — never to mint a new fingerprint.
function reconcileInputFingerprintLegacySha256(verdict) {
  return legacyFingerprintSha256(reconciliationFingerprintPayload(verdict));
}
...
// fingerprint still describes today's verdict. The xxh3 migration changed
// what a FRESH fingerprint looks like, but it must not silently re-open
// every decision a user already made under the old sha256: form — so a
// decision is honoured if it matches EITHER the current xxh128: fingerprint
// (new decisions, and old ones re-sealed since the migration) OR the legacy
// sha256: fingerprint of the SAME verdict content (decisions made before the
// migration and never touched since). Only decisions whose stored value
// matches neither form (content genuinely changed) are treated as stale.
export function isReconciliationDecisionCurrent(verdict, decision) {
  ...
  if (stored === reconcileInputFingerprint(verdict)) return true;
  if (stored.startsWith("sha256:") && stored === reconcileInputFingerprintLegacySha256(verdict)) return true;
  return false;
}
